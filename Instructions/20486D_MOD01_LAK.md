...
10. In **Microsoft Edge**, in the navigation bar, click **Contact** to review its content.
...
5. In the **ActorsRazorPages - Microsoft Visual Studio** window, in **Solution Explorer**, expend **Pages**, expend **Shared** and then click  **_Layout.cshtml**.
...
33. In the dialog box, navigate to **Allfiles\Mod01\Labfiles\01_ActorsRazorPages_begin\Pages**, select **Details.cshtml.cs** and **Details.cshtml**, and then click **Add**. 
...
5. In the **Template** selector, select **Empty (without model)**.
...
7. In the **Add MVC View** dialog box, select the **Use a layout page** check box is selected, and then click **Add**.

8. In the **TestPage.cshtml** code window, select the following code:
  ```cs
       <h2>TestPage</h2>
```
...
15. In the **Animal.cs** code block, place the cursor after the second **{** (opening braces) sign, press Enter, and then type the following code:
  ```cs
       public int Id { get; set; }
...
17. In the **Add New Item - AnimalsMvc** dialog box, click **Interface**. 

18. In the **Add New Item – AnimalsMvc** dialog box, in the **Name** text box, type **IData**, and then click **Add**.

19. In the **IData** interface code block, place the cursor before the word **interface**, and then write **public**.

20. In the **IData** interface code block, place the cursor after the second **{** (opening braces) sign, press Enter, and then type the following code:
  ```cs
...
22. In the dialog box, navigate to **Allfiles\Mod01\Labfiles\03_AnimalsMvc_begin**, click **Data.cs**, and then click **Add**.
...
25. In the **IndexViewModel.cs** code block, place the cursor after the second **{** (opening braces) sign, press Enter, and then type the following code:
  ```cs
...
28. In the **Add Empty MVC Controller** dialog box, in the **Controller name** text box, type **AnimalsController**, and then click **Add**.
...
31. In the **AnimalsController.cs** code block, place the cursor after the second **{** (opening braces) sign, press Enter, and then type the following code:
  ```cs
...
32. In the **AnimalsMvc - Microsoft Visual Studio** window, in **Solution Explorer**, expend **wwwroot**, right-click on the **images**, point to **Add**, and then click  **Existing Item**.
...
36. In the **Template** selector, select **Empty (without model)**.
...
39. In the **AnimalsMvc - Microsoft Visual Studio** window, in the Solution Explorer, under **Controllers**, click **AnimalsController.cs**.

40. In the **AnimalsController.cs** code window, locate the following code, right-click the code, and then click **Add View**.
  ```cs
       public IActionResult Details(int? id)
```
41. In the **Add MVC View** dialog box, ensure that the name in the **View name** text box is **Details**.

42. In the **Template** selector, select **Empty (without model)**.
