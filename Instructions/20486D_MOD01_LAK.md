...
6. Replace the content in the **Location** box with **Allfiles\Mod01\Labfiles\01_ActorsRazorPages_begin**, and then click **OK**.
7. In the **New ASP.NET Core Web Application - ActorsRazorPages** dialog box, in the result pane, ensure that the **Configure for HTTPS** check box is not selected.
...
10. In Microsoft Edge, on the navigation bar, click **Contact** to review its content.
...
2. In the **_ViewStart.cshtml** code window, note that the value of Layout is **"_Layout"**.
    >**Note**: This indicates that all the files inside the **Pages** folder use the same layout file which is **~/Pages/_Layout.cshtml**.
3. In the **ActorsRazorPages - Microsoft Visual Studio** window, in **Solution Explorer**, under **Pages**, click **Contact.cshtml**.
4. In the **Contact.cshtml** code window, examine the Razor code, and then verify that there are no links to .css files.
...
7. In the **ActorsRazorPages - Microsoft Visual Studio** window, in **Solution Explorer**, under **ActorsRazorPages**, expand **wwwroot**, expand **css**, and then click **site.css**.
    >**Note**: This is the Cascading Style Sheets (CSS) file that was applied in the **Layout.cshtml**.

#### Task 3: Add simple functionality
1. In the **ActorsRazorPages - Microsoft Visual Studio** window, in **Solution Explorer**, right-click **Pages**, point to **Add**, and then select **New Item**.
...
```
8. In the **ActorsRazorPages - Microsoft Visual Studio** window, in **Solution Explorer**, right-click **ActorsRazorPages**, point to **Add**, and then select **New Folder**.
9. In the **NewFolder** box, type **Models**, and then press Enter.
10. In the **ActorsRazorPages - Microsoft Visual Studio** window, in **Solution Explorer**, right-click **Models**, point to **Add**, and then select **Class**.
11. In the **Add New Item – ActorsRazorPages** dialog box, in the **Name** box, type **Actor**, and then click **Add**.
12. In the **Actor.cs** code block, place the cursor after the second **{** (opening brace), press Enter, and then type the following code:
  ```cs
...
```
13. In the **ActorsRazorPages - Microsoft Visual Studio** window, in **Solution Explorer**, right-click **Models**, point to **Add**, and then select **New Item**.
...
17. In the **IData** interface code block, place the cursor after the second **{** (opening brace), press Enter, and then type the following code:
  ```cs
...
19. In the dialog box, go to **Allfiles\Mod01\Labfiles\01_ActorsRazorPages_begin**, click **Data.cs**, and then click **Add**.
    >**Note**: Examine the **Data.cs** class content.
20. In the **ActorsRazorPages - Microsoft Visual Studio** window, in **Solution Explorer**, under **wwwroot**, right-click **images**, point to **Add**, and then select **Existing Item**.
21. In the dialog box, go to **Allfiles\Mod01\Labfiles\01_ActorsRazorPages_begin\Images**, select all the images, and then click **Add**.
...
24. In the **ActorsRazorPages - Microsoft Visual Studio** window, in **Solution Explorer**, under **Pages**, right-click **Actors**, point to **Add**, and then select **New Item**. 
...
```
29. In the **Index.cshtml.cs** code block, place the cursor after the second **{** (opening brace), press Enter, and then type the following code:
  ```cs
       private IData _data;
...
```
32. In the **ActorsRazorPages - Microsoft Visual Studio** window, in **Solution Explorer**, under **Pages**, right-click **Actors**, point to **Add**, and then select **Existing Item**.
33. In the dialog box, go to **Allfiles\Mod01\Labfiles\01_ActorsRazorPages_begin\Pages**, select **Details.cshtml.cs** and **Details.cshtml**, and then click **Add**. 
    >**Note**: Examine the content in the **Details.cshtml.cs** and **Details.cshtml** files.
...
35. Ensure that the cursor is at the end of the **using Microsoft.Extensions.DependencyInjection** namespace code, press Enter, and then type the following code:
  ```cs
       using ActorsRazorPages.Models;
```
36. Place the cursor after the **{** (opening brace) of the **ConfigureServices** method, press Enter, and then type the following code:
  ```cs
...
3. In Microsoft Edge, on the navigation bar, click **Test Page** to view its content.
    >**Note**: The browser window displays the **Test Page** title and the text **&quot;This is a Test Page&quot;**. 
4. In the **Test Page** window, on the navigation bar, click **Actors** to view its content.
    >**Note**: The browser window displays the **Index.cshtml** page under the **Actors** folder.
5. In the **Actors** window, select an actor, and then, to go to the **Details** page, click **Details**.
...
#### Task 1: Creating an ASP.NET Web API application
...
6. Replace the content in the **Location** box, with **Allfiles\Mod01\Labfiles\02_CakeStoreApi_begin**, and then click **OK**.
7. In the **New ASP.NET Core Web Application - CakeStoreApi** dialog box, in the result pane, ensure that the **Configure for HTTPS** check box is not selected.
...
4. In the **CakeStoreApi (Running) - Microsoft Visual Studio** window, on the **DEBUG** menu, click **Stop Debugging**.

#### Task 3: Add simple functionality

1. In the **CakeStoreApi - Microsoft Visual Studio** window, in **Solution Explorer**, right-click **CakeStoreApi**, point to **Add**, and then select **New Folder**.
2. In the **NewFolder** box, type **Models**, and then press Enter.
3. In the **CakeStoreApi - Microsoft Visual Studio** window, in **Solution Explorer**, right-click **Models**, point to **Add**, and then select **Class**.
4. In the **Add New Item – CakeStoreApi** dialog box, in the **Name** box, type **CakeStore**, and then click **Add**.
5. In the **CakeStore.cs** code block, place the cursor after the second **{** (opening brace), press Enter, and then type the following code:
  ```cs
       public int Id { get; set; }
       public string CakeType { get; set; }
       public int Quantity { get; set; }
```
6. In the **CakeStoreApi - Microsoft Visual Studio** window, in **Solution Explorer**, right-click **Models**, point to **Add**, and then select **New Item**.
...
10. In the **IData.cs** code block, place the cursor after the second **{** (opening brace), press Enter, and then type the following code:
  ```cs
       List<CakeStore> CakesList { get; set; }
       List<CakeStore> CakesInitializeData();
       CakeStore GetCakeById(int? id);
```
11. In the **CakeStoreApi - Microsoft Visual Studio** window, in **Solution Explorer**, right-click **Models**, point to **Add**, and then select **Existing Item**.
12. In the dialog box, go to **Allfiles\Mod01\Labfiles\02_CakeStoreApi_begin**, click **Data.cs**, and then click **Add**.
    >**Note**: Examine the **Data.cs** content.
13. In the **CakeStoreApi - Microsoft Visual Studio** window, in **Solution Explorer**, right-click **Controllers**, point to **Add**, and then select **Controller**.
...
```
17. In the **CakeStoreApiController.cs** code block, place the cursor after the second **{** (opening brace), press Enter, and then type the following code:
  ```cs
       private IData _data;
...
19. Ensure that the cursor is at the end of the **using Microsoft.Extensions.Options** namespace code, press Enter, and then type the following code:
  ```cs
       using CakeStoreApi.Models;
```
21. Place the cursor after the **{** (opening brace) of the **ConfigureServices** method, press Enter, and then type the following code:
  ```cs
       services.AddSingleton<IData, Data>();
``` 
...
3. In Microsoft Edge, in the address bar, type **http://localhost:***[port]***/api/CakeStore**, and then press Enter.
    >**Note**: The browser displays a list of cake stores in the **JSON** format.
...
6. Replace the content in the **Location** box with **Allfiles\Mod01\Labfiles\03_AnimalsMvc_begin**, and then click **OK**.
7. In the **New ASP.NET Core Web Application - AnimalsMvc** dialog box, in the result pane, ensure that the **Configure for HTTPS** check box is not selected.
8. In the **New ASP.NET Core Web Application** dialog box, in the result pane, click **Web Application (Model-View-Controller)**, and then click **OK**.
9. In the **AnimalsMvc – Microsoft Visual Studio** window, on the **DEBUG** menu, click **Start Debugging**.
10. In Microsoft Edge, on the navigation bar, click **Contact** to review its content.
11. In Microsoft Edge, click **Close**.
...
2. In the **_ViewStart.cshtml** code window, note that the value for **Layout** is **"_Layout";**.
...
```
2. Place the cursor after the **}** (closing brace) of the located code, press Enter, and then type the following code:
  ```cs
...
```
3. Right-click the code you just added, and then select **Add View**.
...
6. In the **Add MVC View** dialog box, ensure that the **Create as a partial view** check box is not selected and the **Use a layout page** check box is selected, and then click **Add**.
...
```
12. In the **AnimalsMvc – Microsoft Visual Studio** window, in **Solution Explorer**, right-click **Models**, point to **Add**, and then select **Class**.
13. In the **Add New Item – AnimalsMvc** dialog box, in the **Name** box, type **Animal**, and then click **Add**.
14. In the **Animal.cs** code block, place the cursor after the second **{** (opening brace), press Enter, and then type the following code:
  ```cs
       public int Id { get; set; }
...
```
15. In the **AnimalsMvc - Microsoft Visual Studio** window, in **Solution Explorer**, right-click **Models**, point to **Add**, and then select **New Item**. 
...
19. In the **IData** interface code block, place the cursor after the second **{** (opening brace), press Enter, and then type the following code:
  ```cs
       List<Animal> AnimalsList { get; set; }
       List<Animal> AnimalsInitializeData();
       Animal GetAnimalById(int? id);
```
20. In the **AnimalsMvc  – Microsoft Visual Studio** window, in **Solution Explorer**, right-click **Models**, point to **Add**, and then select **Existing Item**.
21. In the dialog box, go to **Allfiles\Mod01\Labfiles\03_AnimalsMvc_begin**, click **Data.cs**, and then click **Add**.
    >**Note**: Examine the content in **Data.cs**.
22. In the **AnimalsMvc - Microsoft Visual Studio** window, in **Solution Explorer**, right-click **Models**, point to **Add**, and then select **Class**.
23. In the **Add New Item – AnimalsMvc** dialog box, in the **Name** box, type **IndexViewModel**, and then click **Add**.
24. In the **IndexViewModel.cs** code block, place the cursor after the second **{** (opening brace), press Enter, and then type the following code:
  ```cs
       public List<Animal> Animals { get; set; }
```
25. In the **AnimalsMvc - Microsoft Visual Studio** window, in **Solution Explorer**, right-click **Controllers**, point to **Add**, and then select **Controller**.
...
```
30. In the **AnimalsController.cs** code block, place the cursor after the second **{** (opening brace), press Enter, and then type the following code:
  ```cs
...
``` 
31. In the **AnimalsMvc - Microsoft Visual Studio** window, in **Solution Explorer**, expand **wwwroot**, right-click **images**, point to **Add**, and then select **Existing Item**.
32. In the dialog box, go to **Allfiles\Mod01\Labfiles\03_AnimalsMvc_begin\Images**, select all the images, and then click **Add**.
33. In the **AnimalsController.cs** code window, locate the following code, right-click the code, and then select **Add View**.
  ```cs
       public IActionResult Index()
```
...
36. In the **Add MVC View** dialog box, ensure that the **Create as a partial view** check box is not selected and the **Use a layout page** check box is selected, and then click **Add**.
37. In the **Index.cshtml**, erase all the content in the window, and then type the following code:
  ```cs
...
```
38. In the **AnimalsMvc - Microsoft Visual Studio** window, in **Solution Explorer**, under **Controllers**, click **AnimalsController.cs**.
39. In the **AnimalsController.cs** code window, locate the following code, right-click the code, and then select **Add View**.
  ```cs
       public IActionResult Details(int? id)
```
40. In the **Add MVC View** dialog box, ensure that the name in the **View name** box is **Details**.
41. In the **Add MVC View** dialog box, ensure that the **Empty (without model)** template is selected.
42. In the **Add MVC View** dialog box, ensure that the **Create as a partial view** check box is not selected and the **Use a layout page** check box is selected, and then click **Add**.
43.  In the **Details.cshtml**, erase all the content in the window, and then type the following code:
  ```cs
...
49. Place the cursor after the **{** (opening brace) of the **ConfigureServices** method, press Enter, and then type the following code:
  ```cs
       services.AddSingleton<IData, Data>();
``` 
#### Task 4: Run the application
...
3. In Microsoft Edge, on the navigation bar, click **Test Page** to review its content.
    >**Note**: The browser window displays the text **&quot;This is a Test Page&quot;**.
4. In the **Test Page** window, on the navigation bar, click **Animals** to view its content.
    >**Note**: The browser window displays the **Index.cshtml** page, under the **Animals** folder.
5. In the **Animals** window, select an animal, and then, to go to the **Details** page, click **Details**.
