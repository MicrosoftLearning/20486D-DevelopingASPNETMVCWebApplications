...
12. In the **IData** class code block place the mouse cursor before the word interface and write **public**.
14. In the **IData** class code block, press Enter and type the following code.
...
17. In the **ActorsRazorPages – Microsoft Visual Studio** window, in the Solution Explorer pane, right-click **Pages** folder, right click, point to **Add** , and select **New folder** In the Name box of the New Folder , type **Actors**.
 ...
 19. In the **Name** box of the **Add New Item – ActorsRazorPages** dialog box, type **Index.cshtml**.
 20. In the **Index.cshtml.cs** code window, select the **OnGet** method code  , and then press Delete.
...
25. In the **ActorsRazorPages – Microsoft Visual Studio** window, in the Solution Explorer pane, select **Startup.cs**  place the mouse cursor at the end of the **using Microsoft.Extensions.DependencyInjection;** namespace code, press Enter, and then type the following code.
  ...
  ```cs
		using ActorsRazorPages.Models;
```
26. In the **Startup.cs** , In the  **ConfigureServices** method above **services.AddMvc();** method call type the following code.
```cs
services.AddSingleton<IData, Data>();
```
27. In the **ActorsRazorPages – Microsoft Visual Studio** window, in the Solution Explorer pane, expand **pages** folder, select **_Layout.cshtml** code window, in the **ul** element, type the following code.
  ```cs
...
4. In the **Test Page** window, click **Actors** in the navigation bar to view its contents.
    ...
    >**Note:** The table that  is displayed on the browser is the Index.cshtml page that you just added.

5. In the **Actors** window, click on the **Details** word to go to the details page.
   >**Note:** The content that  is displayed on the browser is the Details.cshtml page that you just added.
 
6. note that the standard site layout and styles have been applied in all the pages.
...
3. In the navigation pane of the **New Project** dialog box, expand **Installed**, and then click **Visual C#**.
4. In the result pane of the **New Project** dialog box, click **ASP.NET Core Web Application**.
6. In the Name box, replace **WebApplication1** with **CakeStoreApi**, and then click **OK**.
5. In the result pane of the **New ASP.NET Core Web Application**  dialog box, click **Web API**,  and then click **OK**.
6. On the **DEBUG** menu of the **CakeStoreApi –  Microsoft Visual Studio** window, click **Start Debugging**.
7. On the home page of the **http://localhost:[port]/** window, review its contents.
...
1. In the **CakeStoreApi  – Microsoft Visual Studio** window, in the Solution Explorer pane, right-click **CakeStoreApi**, point to **Add**, and then click **New Folder**.
In the **Name** box of the **New Folder** , type **Models**.
...
```
   >**Note:** The attributes that are written above indicates the URL that the user should write to get to the relevant method result.
   >
...
6. In the result pane of the **New ASP.NET Core Web Application**  dialog box, click **Web Application (Model-View-Controller)**,
...
1. In the Solution Explorer pane, expand **Views** folder and right-click **Home**, point to **Add**, and then click **View**. In the **View name** box of the **Add MVC View** dialog box, replace **View** with **TestPage** In the **Template** selector, select **Empty (without model)**, and check the **Use a layout page** check box, and then click **Add**.
2. In the TestPage.cshtml **below**  the **h2** element  code window, type the following code.

  ```cs
       <h1>This is a Test Page</h1>
```
3. In the Solution Explorer pane, expand **Controllers**, and then click **HomeController.cs** class code block, press Enter at the end of the **Error()** method curly bracket and type the following code.
...
```
4. In the Solution Explorer pane, under Views, expand **Shared**, and then click **_Layout.cshtml.**
5.  In the _Layout.cshtml code window, in the **ul** element, type the following code.
  ```cs
     <li><a asp-area="" asp-controller="Home" asp-action="TestPage">Test Page</a></li>
```
6. In the **AnimlasMvc  – Microsoft Visual Studio** window, in the Solution Explorer pane, right-click **Models**, point to **Add** , and then click **class**  In the **Name** box of the **Add New Item – AnimlasMvc** dialog box, type **Animal**, and then click **Add**.
    
 7. In the **Animal** class code block, press Enter, type the following code, and then press Enter.
...
8. In the **AnimlasMvc  – Microsoft Visual Studio** window, in the Solution Explorer pane, right-click **Models**, point to **Add** , and **New Item** and  then click **Interface**  In the **Name** box of the **Add New Item – AnimlasMvc** dialog box, type **IData**, and then click **Add**.

9. In the **IData** class code block place the mouse cursor before the word interface and write **public** and then inside the class block, type the following code, and then press Enter.
```cs
        List<Animal> AnimalsList { get; set; }
        List<Animal> AnimalsInitializeData();
        Animal GetAnimalById(int? id);
```
10. In the **AnimlasMvc  – Microsoft Visual Studio** window, in the Solution Explorer pane, right-click **Models** folder, right click, point to **Add** , and select  **Existing Item**, in the dialog box navigate to **Allfiles/Mod01/Labfiles/AnimalsMvc**, click **Data.cs**, and then click **Add**. note that the namespace project name identical to your project name.
11. In the **AnimlasMvc  – Microsoft Visual Studio** window, in the Solution Explorer pane, right-click **Models**, point to **Add** , and **New Item** and  then click **Class**  In the **Name** box of the **Add New Item – AnimlasMvc** dialog box, type **IndexViewModel**, and then click **Add**.
12.  In the **IndexViewModel** class code block, press Enter, type the following code, and then press Enter.
  ```cs
      public List<Animal> Animals { get; set; }
```

13. In the **AnimlasMvc  – Microsoft Visual Studio** window, in the Solution Explorer pane, right-click **Controllers**, point to **Add**, and then click **Controller** and  then click **MVC Controller - Empty**  In the **Name** box of the **Add Empty MVC Controller** dialog box, type **AnimalsController**.
14.  In the AnimalsController.cs  place the mouse cursor at the end of the **using Microsoft.AspNetCore.Mvc;** namespace code, press Enter, and then type the following code.

  ```cs
		using AnimlasMvc.Models;
```
15. In the **AnimalsController.cs** class code block, press Enter, type the following code, and then press Enter.
...
``` 
   ...
   16. In the **AnimlasMvc  – Microsoft Visual Studio** window, in the Solution Explorer pane, click on the **wwwroot** folder, and then ,on the **images** folder, right click, point to **Add** , and select  **Existing Item**, in the dialog box navigate to **Allfiles/Mod01/Labfiles/AnimalsMvc/Images**, select all the images, and then click **Add**.
   17. In the AnimalsController.cs code window, locate the following code, right-click the code, and then click **Add View**.

  ```cs
       public IActionResult Index()
```
18. In the **Add MVC View** dialog box, ensure that the name in the **View name** box is **Index**.
19. In the **Template** selector, select **Empty (without model)**.
20. In the **Add MVC  View** dialog box, ensure that the **Use a layout** check boxes is selected, and then click **Add**.
21. In the **Index.cshtml** erase all the content in the  window, and type the following code.
...
```
 ...
 22. In the AnimalsController.cs code window, locate the following code, right-click the code, and then click **Add View**.

  ```cs
       public IActionResult Details(int? id)
```
23. In the **Add MVC View** dialog box, ensure that the name in the **View name** box is **Details**.
24. In the **Template** selector, select **Empty (without model)**.
25. In the **Add MVC  View** dialog box, ensure that the **Use a layout** check boxes is selected, and then click **Add**.
26.  In the **Details.cshtml** erase all the content in the  window, and type the following code.
...
```
 ...
 >**Note:** The asp-action Tag Helpers is new feature in ASP.NET Core. Tag Helpers allow server-side code to render HTML elements in Razor files.
 >
27. In the Solution Explorer pane, under Views, expand **Shared**, and then click **_Layout.cshtml.**
28. In the _Layout.cshtml code window, in the **ul** element, type the following code.
  ```cs
     <li><a asp-area="" asp-controller="Animals" asp-action="Index">Animals</a></li>
```
 29. In the **AnimlasMvc  – Microsoft Visual Studio** window, in the Solution Explorer pane, select **Startup.cs**  place the mouse cursor at the end of the **using Microsoft.Extensions.DependencyInjection;** namespace code, press Enter, and then type the following code.

  ```cs
		using AnimlasMvc.Models;
```
   
30. In the **Startup.cs** , Inside  **ConfigureServices** method **above** **services.AddMvc();** method  call type the following code.
```cs
services.AddSingleton<IData, Data>();
``` 
