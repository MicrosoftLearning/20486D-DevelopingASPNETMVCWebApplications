...
11. Place the cursor before the located code, press the Up Arrow key, and then type the following code:
  ```cs
       [Authorize]
``` 
#### Task 2: Role-based Policy Authentication
...
4. In the **Library - Microsoft Visual Studio** window, in **Solution Explorer**, under **Controllers**, and then click **AccountController.cs**.
...
7. In the **AccountController.cs** code window, in the **Register** action code block, select the following code:
  ```cs
       var result = await _userManager.CreateAsync(user, registerModel.Password);
       if (result.Succeeded)
       {
...
#### Task 3: Claim-based Policy Authentication
...
6. In the **Library - Microsoft Visual Studio** window, in **Solution Explorer**, under **Controllers**, and then click **AccountController.cs**.
...
9. In the **AccountController.cs** code window, in the **Register** action code block, select the following code:
  ```cs
       var result = await _userManager.CreateAsync(user, registerModel.Password);
        if (result.Succeeded)
        {
