...
```
 ...
 Put the cursor at the end of the line that is above the located code, and write the code below:

```cs
    public void Configure(IApplicationBuilder app, IHostingEnvironment env, ILogger log)
```

21. Inside the **Configure** method, locate the following code:

```cs
    await context.Response.WriteAsync("Hello World!");
```

22. Press enter, and type the code below:
...
26. In the **Solution Explorer**, double click the new created **XXXX-XX-XX--XX-XX-XX.log** file.

27. See the **[DateTime]: Logged Line** text shown inside the file.
