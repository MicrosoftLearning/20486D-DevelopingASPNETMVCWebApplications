...
4. In the **Task Runner Explorer** pane, under **Tasks**, right-click **js-watcher**, and then click **Run**.
    ...
    >**Note:** In Solution Explorer, under **wwwroot**, a new js File has been added named **vendor.min.js** and under **lib** folder, and a new js File has been added named **script.min.js** and under **script** folder.
...
1. In the **IceCreamCompany - Microsoft Visual Studio (Administrator)** window, in **Solution Explorer**, under **Styles** click **main.scss**.

2. In the **main.scss** code window, select the following code: 
  ```cs
       body {
       }
```
...
2. In the **main.scss** code window, locate the following code: 
  ```cs
       @mixin normalized-image() {
          width: 100%;
          height: auto;
...
5. Place the cursor immediately after the last typed **}** (closing bracket) sign, press Enter twice, and then type the following code:
  ```cs
       nav {
           width: 450px;
       }
...
5. On the **Buy ice cream** page, in the **Buy bulk(lbs)** list, select _&lt;A bulk size of your choice&gt;._
