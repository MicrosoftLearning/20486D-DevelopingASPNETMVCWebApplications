...
1. Navigate to **Allfiles\Mod09\Labfiles\01_IceCreamCompany_begin**.
...
3. In the command prompt, type the following command, and then press Enter.
  ```cs
       npm install
```
4. On the taskbar, click **File Explorer**, and then double-click **IceCreamCompany.sln**.

8. In the **IceCreamCompany - Microsoft Visual Studio** window, in **Solution Explorer**, click **package.json**.
...
4. In **Task Runner Explorer**, under **Tasks**, right-click **min:js**, and then click **Run**.
    ...
    >**Note:** In **Solution Explorer**, under **wwwroot**, under **script** folder, a new **script.min.js** file has been added.
...
17. In the **Add MVC View** dialog box, ensure that the name in the **View name** text box is **Buy**.
...
7. In the **Buy.cshtml** code window, place the cursor at the beginning of the document, type the following code, and then press Enter.
  ```cs
...
``` 
10. In the **Buy.cshtml** code window, in the **DIV** element, type the following code:
  ```cs
