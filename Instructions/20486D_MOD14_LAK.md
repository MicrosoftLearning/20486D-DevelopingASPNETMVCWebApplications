...
2. In File Explorer, browse to **Allfiles\Mod14\Labfiles\01_Underwater_begin\Underwater**, and then copy the address in the address bar.
...
1. Browse to **Allfiles\Mod14\Labfiles\01_Underwater_begin**, and then double-click **Underwater.sln**.
...
17. In the  **Publish** window, under the **Default Connection String**, select the **Use this connection string at runtime** check box.

18. In the  **Publish** window, in the box below **Default Connection String**, type:
  ```cs
...
20. In the **Publish** window, expand **Entity Framework Migration**, and in the box under **Apply this migration on publish**, type:
  ```cs
       Server=tcp:Underwater{unique name}.database.windows.net,1433;Initial Catalog=UnderwaterDB;Persist Security Info=False;User ID=Underwateradmin;Password=p@@sw0rd;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;
```
...
23. In the **Pick a publish target** dialog box, verify that **Underwater {unique name}** is selected in the drop-down list, and then click **Publish**.
...
21. In the **UnderwaterRepository.cs** code window, select the following code:
  ```cs
...
26. In the **UnderwaterRepository.cs** code window, select the following code:
  ```cs
...
32. On the **Package Manager Console** tab, type the following command, and then press Enter.
  ```cs
