...
4. In the **ElectricStore - Microsoft Visual Studio** window, in Solution Explorer, under **Controllers**, click **ShoppingCardController.cs**.

5. In the **ShoppingCardController.cs** code window, locate the following code:
  ```cs
      using Microsoft.AspNetCore.Mvc;
```
6. Ensure that the cursor is at the end of the **Microsoft.AspNetCore.Mvc** namespace, press Enter, and then type the following code:
  ```cs
      ...
      using Newtonsoft.Json;
```
7. In the **ShoppingCardController.cs** code block, locate the following code:
  ```cs
      public IActionResult Index()
      {
           return View();
...
```
9. In the **ShoppingCardController.cs** code block, in the **Index** action code block, select the following code:
  ```cs
      return View();
```
...
11. In the menu bar, click **My Shopping Card**.
    ...
    >**Note**: Examine the browser contents. The browser displays the **ShoppingCard.cshtml** file content, rendered by **_Layout.cshtml**, and the cached contents are the products that were selected by you in the **"Add Products to Shopping List"** page.
...
14. On the menu bar, click **My Shopping Card**.
...
17. On the menu bar, click **My Shopping Card**.
...
7. In the second **Microsoft Edge** window, in the address bar, type **http://localhost:[port]/ShoppingCard/Chat**, and then press Enter.
