...
1. Open Microsoft Visual Studio 2017 and create a new ASP.NET Core web application with the following information:
...
2. In the **Startup** class, delete all the comments.
...
5. In the **Configure** method, call the **Run** method of the **app** parameter. To the **Run** method, pass an async lambda expression with the following information:
    - Expression: **async (context) => { }**

6. In the async **lambda expression** code block, call the **context.Response.WriteAsync** method by using the **await** operator. To the **WriteAsync** method, pass **This text was generated by the app.Run middleware.** as a parameter. 
...
12. Copy the content of the **html-text.txt** file with the following information:
...
4. Select **Basketball**, and then click **Submit Poll**.
    >**Note**: The browser displays the text **Selected value is: Basketball**.
...
8. In the **app.Use** middleware call, comment out the **ELSE** statement that would run if the **"favorite" query string** parameter did not exist.
...
13. Uncomment the **ELSE** statement that would run if the **"favorite" query string** parameter did not exist.

>**Result**: At the end of this exercise, you will be able to create custom middleware and receive submitted form data in it.
...
4. In the **IPollResultsService** interface, declare a method with the following information:
    - Return type: **void**
    - Name: **AddVote**
    - Parameter:
        - Name: **game**
...
2. Start the application without debugging.
    >**Note**: The browser displays the text **This text was generated by the app.Run middleware. wwwroot folder path:** *[local path to your wwwroot folder]*.
