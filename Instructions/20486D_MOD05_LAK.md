...
3. Place the mouse cursor at the end of the code, press Enter, type the following code, and press enter again.
```cs
    ICityProvider _cities;
```

4. In the **CityController.cs** code window, select the following code. 
```cs
...
6. In the **CityController.cs** code window, place the mouse cursor within the **CityController** constructor code block, and then type the following code. 
```cs
    _cities = cities;
```

7. In the **CityController.cs** code window, select the following code. 
```cs
...
9. In the **CityController.cs** code window, locate the following code. 
```cs
    public IActionResult ShowDataForCity(string city)
    {
```
...
11. Right click on the **ShowDataForCity** method name, and click **Add View...**.

12. In the **Add MVC View** dialog window verify that none of the checkboxes are selected, and press **Add**.
...
16. In the **ShowDataForCity.cshtml** code window, in the **BODY** element,  type the following code.
```cs
    <div>
```
...
```cs
    ...
    <p>Area Code: @ViewBag.City._timeZone</p>
```
...
20. In the Solution Explorer pane of the **CitiesDetails - Microsoft Visual Studio** window, in the **Controllers** folder, click **CityController.cs**.


21. In the **CityController.cs** code window, select the following code. 
```cs
...
6. In the Solution Explorer pane of the **CitiesDetails - Microsoft Visual Studio** window, inside the **Views** folder, in the **City** subfolder, click **ShowDataForCity.cshtml**.

7. In the **ShowDataForCity.cshtml** code window, locate the following code. 
```cs
    <span><img src="@Url.Action("GetImage", new {cityName = ViewBag.City })" /></span>
```
