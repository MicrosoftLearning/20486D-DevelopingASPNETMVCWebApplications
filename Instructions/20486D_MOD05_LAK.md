...
1. Go to **Allfiles\Mod05\Labfiles\01_CitiesDetails_begin**, and then double-click **CitiesDetails.sln**.

2. In the **CitiesDetails - Microsoft Visual Studio** window, in **Solution Explorer**, expand **Services**, and then click **CityProvider.cs**.
...
5. In the **CitiesDetails - Microsoft Visual Studio** window, in **Solution Explorer**, under **Services**, click **CityDetails.cs**.

6. In the **CityDetails.cs** code window, locate the following code:
```cs
...
7. Place the cursor within the **CityDetails** constructor code block, and then type the following code:
```cs
    Country = country;
    Name = cityName;
    TimeZone = timeZone;
...
8. In the **CitiesDetails - Microsoft Visual Studio** window, in **Solution Explorer**, under **Services**, click **CityPopulation.cs**.
...
11. In the **CitiesDetails - Microsoft Visual Studio** window, in **Solution Explorer**, expand **Controllers**, and then click **CityController.cs**.
...
13. In the **CitiesDetails - Microsoft Visual Studio** window, in **Solution Explorer**, under **Controllers**, click **CityController.cs**.
...
6. In **Add New Item – CitiesDetails** dialog box, in the result pane, click **Razor View Imports**, and then click **Add**.
...
```cs
    <div>
        <h2>
            <a asp-action="ShowDataForCity" asp-route-cityname=@ViewBag.CurrentCityDetails.Name>@ViewBag.CurrentCityDetails.Name -     @ViewBag.CurrentCityDetails.Country</a>
        </h2>
        <img src="@Url.Action("GetImage", new {cityName = ViewBag.CurrentCityDetails.Name })" />
    </div>
```
...
2. In the **CitiesDetails - Microsoft Visual Studio** window, on the **DEBUG** menu, click **Start Debugging**.
    >**Note**: The browser displays a "Select City" title and a list of rendered partial views, each displays the city's title, country and mini-map image.
