...
3. Locate the following code, and then select the code:
```cs
...
```cs
    await context.Response.WriteAsync("wwwroot folder path: " + env.WebRootPath);
```

7. On the **DEBUG** menu of the **PollBall –  Microsoft Visual Studio** window, click **Start Debugging**.
> **Note**: The local path to your wwwroot folder is shown.

8. In the **Microsoft Edge** window, click **Close**.

9. On the **Debug** Menu, click **Stop Debugging**.

10. Locate the following code, and then select the code:
```cs
    await context.Response.WriteAsync("wwwroot folder path: " + env.WebRootPath);
```
