...
``` 
2. In the **IPollResultsService.cs** code window, locate the following code, and then select the code.
```cs
...
```
6.	In the **Startup.cs** code window, locate the following code, and then select the code.
```cs
await context.Response.WriteAsync("Selected Value is = " + selectedValue);
```
...
```
9. Place the mouse cursor within the **ConfigureServices** method block, press enter, and then type the following code. 
```cs
