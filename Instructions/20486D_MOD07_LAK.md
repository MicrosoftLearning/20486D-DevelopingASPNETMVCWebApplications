...
6. In the **Startup.cs** code window, in the **ConfigureServices** method, place the cursor after the **{** (opening braces) sign, press Enter, type the following code, and then press Enter.
  ```cs
      services.AddDbContext<CupcakeContext>(options =>
             options.UseInMemoryDatabase("CupcakesDb"));
```

7. In the **Startup.cs** code window, select the following code:
  ```cs
...
9. In the **Startup.cs** code window, place the cursor after the **{** (opening braces) sign of the **Configure** method, press Enter, type the following code, and then press Enter.
  ```cs
...
3. Place the cursor immediately after the last typed **}** (closing braces) sign, press Enter twice, and then type the following code:
  ```cs
...
>**Results**: After completing this exercise, you will be able to add Entity Framework Core to the Cupcake application. 
...
18. Ensure that the cursor is at the end of the **CupcakeRepository** method code block, press Enter twice, and then type the following code:
  ```cs
      public void CreateCupcake(Cupcake cupcake)
      {
         ...
         {
            Cupcake lastCupcake = _context.Cupcakes.LastOrDefault();
            cupcake.CupcakeId = lastCupcake.CupcakeId + 1;
            ...
            using (var memoryStream = new MemoryStream())
            {
             ...
             _context.Add(new Cupcake() {CupcakeId = cupcake.CupcakeId, Bakery = cupcake.Bakery, BakeryId = cupcake.BakeryId, CupcakeType = cupcake.CupcakeType, Description = cupcake.Description, GlutenFree = cupcake.GlutenFree, ImageMimeType = cupcake.ImageMimeType, ImageName = cupcake.ImageName, PhotoAvatar = cupcake.PhotoAvatar, PhotoFile = cupcake.PhotoFile, Price = cupcake.Price });
...
5. Ensure that the cursor is at the end of the **CreatePost** action code block with **HTTP verb POST**, press Enter, and then type the following code:
  ```cs
       [HttpGet]
...
7. Ensure that the cursor is at the end of the **Edit** action code block with **HTTP verb GET**, press Enter, and then type the following code:
  ```cs
...
9. Ensure that the cursor is at the end of the **EditPost** action code block with **HTTP verb POST**, press Enter, and then type the following code:
  ```cs
       [HttpGet]
...
11. Ensure that the cursor is at the end of the **Delete** action code block with **HTTP verb GET**, press Enter, and then type the following code:
  ```cs
...
6. In the **Startup.cs** code window, locate and delete the following code:
  ```cs
      cupcakeContext.Database.EnsureDeleted();
      cupcakeContext.Database.EnsureCreated();
```

7. In the **Cupcakes - Microsoft Visual Studio** window, in **Solution Explorer**, under **Controllers**, and then click **CupcakeController.cs**.

7. In the **CupcakeController.cs** code window, locate and delete the following code:
  ```cs
      Cupcake lastCupcake = _context.Cupcakes.LastOrDefault();
      cupcake.CupcakeId = lastCupcake.CupcakeId + 1;
```
8. In the **CupcakeController.cs** code window, select the following code:
  ```cs
       _context.Add(new Cupcake() {CupcakeId = cupcake.CupcakeId, Bakery = cupcake.Bakery, BakeryId = cupcake.BakeryId, CupcakeType = cupcake.CupcakeType, Description = cupcake.Description, GlutenFree = cupcake.GlutenFree, ImageMimeType = cupcake.ImageMimeType, ImageName = cupcake.ImageName, PhotoAvatar = cupcake.PhotoAvatar, PhotoFile = cupcake.PhotoFile, Price = cupcake.Price });
```
...
1. In the **Cupcakes - Microsoft Visual Studio** window, in **Solution Explorer**, right-click **Cupcakes**, point to **Edit Cupcakes.csproj**, and then click.
...
6. In the opened folder, select the address in the address bar, type **cmd**, and then press Enter.

7. In the command Line window, type the following command.
  ```cs
     dotnet ef
```
...
8. In the command Line window, type the following command.
  ```cs
     dotnet ef dbcontext info
```

9. In the command Line window, locate the following line.
  ```cs
...
10. In the command Line window, type the following command.
  ```cs
     dotnet ef database drop
```
>**Note:** To ensure that there are no database conflicts.

11. In the command Line window, type the following command.
  ```cs
...
12. In the command Line window, type the following command, and then **close** the window.
  ```cs
     dotnet ef database update
```
13. In the **Cupcakes - Microsoft Visual Studio** window, on the **View** menu, click **SQL Server Object Explorer**.

14. In the **Cupcakes - Microsoft Visual Studio** window, in **SQL Server Object Explorer**, expand (localdb)\MSSQLLocalDB , expand Databases, and then expand BakeriesDb.
...
>**Results**: After completing this exercise, you should have created a Cupcakes application in which users can submit, edit, delete and view cupcales details.
