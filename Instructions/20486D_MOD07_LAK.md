...
9. In the **Startup.cs** code window, place the cursor after the first **{** (opening braces) sign of the **Configure** method, press Enter, type the following code, and then press Enter.
  ```cs
...
2. Place the mouse cursor at the end of the located code, type the following code, and then press Enter.
  ```cs
       [HttpGet]
...
7. In the **Cupcakes - Microsoft Visual Studio** window, in **Solution Explorer**, under **Repositories**, and then click **CupcakeRepository.cs**.

8. In the **CupcakeRepository.cs** code window, locate and delete the following code:
  ```cs
      Cupcake lastCupcake = _context.Cupcakes.LastOrDefault();
      cupcake.CupcakeId = lastCupcake.CupcakeId + 1;
```
9. In the **CupcakeRepository.cs** code window, select the following code:
  ```cs
       _context.Add(new Cupcake() {CupcakeId = cupcake.CupcakeId, Bakery = cupcake.Bakery, BakeryId = cupcake.BakeryId, CupcakeType = cupcake.CupcakeType, Description = cupcake.Description, GlutenFree = cupcake.GlutenFree, ImageMimeType = cupcake.ImageMimeType, ImageName = cupcake.ImageName, PhotoAvatar = cupcake.PhotoAvatar, PhotoFile = cupcake.PhotoFile, Price = cupcake.Price });
```
...
7. In the command Line window, type the following command, and then press Enter.
  ```cs
     dotnet ef
```
...
8. In the command Line window, type the following command, and then press Enter.
  ```cs
...
10. In the command Line window, type the following command, and then press Enter.
  ```cs
     dotnet ef database drop
```

11. In the command Line window, type the following command, and then press Enter.
  ```cs
     y
```
>**Note:** To ensure that there are no database conflicts.

12. In the command Line window, type the following command, and then press Enter.
  ```cs
...
13. In the command Line window, type the following command, and then **close** the window.
  ```cs
