...
22.  In the **Cupcakes - Microsoft Visual Studio** window, in **Solution Explorer**, expand **Controllers**, and then click **CupcakeController.cs**. 

23.  In the **CupcakeController.cs** code window, locate the following code in the **EditPost** method code block:
```cs
    var cupcakeToUpdate = _repository.GetCupcakeById(id);
     bool isUpdated = await TryUpdateModelAsync<Cupcake>(
                              cupcakeToUpdate,
...
```cs
    ...
    bool isUpdated = await TryUpdateModelAsync<Cupcake>(
                                     ...
                                     c => c.CaloricValue);
```
25. In the **Cupcakes - Microsoft Visual Studio** window, on the **TOOLS** menu, point to **NuGet Package Manager**, and then click **Package Manager Console**.

26. In the **Package Manager Console** tab, type the following command, and then press Enter.
  ```cs
